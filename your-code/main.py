...
Original file is located at
    https://colab.research.google.com/drive/1peTXOa2fz246pUTTfAPZGOXVewwXaG4o
...
# Your code here
aw = pd.read_csv("/content/austin_weather.csv")
aw
...
aw['DewPointAvgF'].unique()

# Because there is a non value (-) in the column, pandas identify all as objec.
...
aw[wrong_type_columns] = aw[wrong_type_columns].apply(pd.to_numeric, errors='coerce')
...
# Your code here
missing_values = aw.isnull().sum(axis=1)

# number of rows with missing data
print("Number of rows with missing_values:", missing_values[missing_values > 0].shape[0])
...
ratio_missing_to_total = df.isnull().any(axis=1).mean()

print("Ratio of missing rows to total rows: {:.2%}".format(ratio_missing_to_total))
...
# Your code here
# Calculate the ratio of missing values in 'PrecipitationSumInches' column to total rows
ratio_missing_precipitation = aw['PrecipitationSumInches'].isna().mean()

# Print the ratio
print("Ratio of missing values in 'PrecipitationSumInches' column: {:.2%}".format(ratio_missing_precipitation))
...
# Your code here
austin_fixed = aw.interpolate(inplace=False)
...
missing_values_fixed = austin_fixed.isnull().sum()
...
# Your code here:
events_counts = aw['Events'].value_counts()
events_counts
...
# Your answer:
max_events_per_day = aw['Events'].value_counts().max()
...
# Assuming you have already performed linear interpolation
austin_fixed = aw.interpolate(inplace=False)

# List of all event types
event_types = ['Rain', 'Snow', 'Fog', 'Thunderstorm']

# Populate actual values in the dummy columns of austin_fixed
for event in event_types:
...
# Your code here
aw.drop("Events", axis=1, inplace=True)
...
X = ['Date', 'TempHighF', 'TempLowF', 'DewPointHighF', 'DewPointAvgF', 'DewPointLowF', 'HumidityHighPercent', 'HumidityAvgPercent', 'HumidityLowPercent', 'SeaLevelPressureHighInches', 'SeaLevelPressureAvgInches', 'SeaLevelPressureLowInches', 'VisibilityHighMiles', 'VisibilityAvgMiles', 'VisibilityLowMiles', 'WindHighMPH', 'WindAvgMPH', 'WindGustMPH', 'Snow', 'Fog', 'Rain', 'Thunderstorm']

label = pd.Series(aw["TempAvgF"], name = "label")

# Your code here:
from sklearn.model_selection import train_test_split

X_train, X_test, y_train, y_test = train_test_split(austin_fixed, label, test_size = 0.20, random_state = 0)
